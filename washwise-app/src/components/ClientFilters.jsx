...
//todo placeholder da pesquisa e seta do drop
    ...
    <div className='h-full w-full grid grid-cols-3'>
        ...
        <div className="row-start-1 relative w-[70%] cursor-pointer">
          <div className="bg-[#C1C0C0] rounded-2xl p-3 shadow-sm flex items-center" onClick={toggleOptions}>
            <input
              ...
              value={searchType === "name" ? "Nome de Cliente" : searchType === "number" ? "Número de Cliente" : "Número de Talão"}
              readOnly
            />
          ...
          {showOptions && (
            <ul className="absolute top-full left-0 w-full bg-[#C1C0C0] rounded-2xl shadow-lg mt-1 max-h-[200px] overflow-y-auto z-50">
              {["name", "number", "receipt"].map((option, index) => (
                <li
                  key={index}
                ...
                >
                  <span>{option === "name" ? "Nome de Cliente" : option === "number" ? "Número de Cliente" : "Número de Talão"}</span>
                </li>
              ))}
            </ul>
          )}
      ...
      <div className="h-full ml-1 flex-col flex justify-center">
        <div className="flex w-[70%] text-3xl overflow-clip mt-3 mb-1">
          <p>{`${searchType === "name" ? "Nome de Cliente" : searchType === "number" ? "Número de Cliente" : "Número de Talão"}`}</p>
        </div>
      ...
      {/* Coluna extra (se necessário, pode ser deixada em branco) */}
